...
function VariableCosts({project}) {

    const dispatch = useDispatch()
    const [visibility, setVisibility] = useState("hidden")


    const setVariableCosts = (value) => {
        let newProject = {
            id: project.id,
            title: project.title,
    ...
    return (
        <div className={s.inputWrapper}>
            <div className={s.name}>Введите переменные издержки</div>
            <div className={s.rightFlex}>
                <input className={s.inputArea} onChange={event => setVariableCosts(event.target.value.replace(/[^0-9,.]/g,"").replace(/([,.]+)\1/g, ".").replace(",", "."))}
                 type="text" placeholder="₽" value={!project.costs.variableCosts ?  "" : String(project.costs.variableCosts).replace(",", ".")}></input>
                <button className={s.button} onClick={() => setVariableCosts("")}>СБРОСИТЬ</button>
                <div onClick={() => setVisibility("visible")} className={s.info} >?</div>
            </div>
            <InfoPopup text={infoPopups.variableCosts} setVisibility={setVisibility} visibility={visibility}/>
        </div>
    )
}
